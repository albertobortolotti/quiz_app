...
nl(1)
email_input = st.text_input("Inserisci la tua email per iniziare:", key="email_input")
...
switch_page = st.button("Inizia", disabled=not st.session_state.get('email_valid', False))
# Validazione e archiviazione dell'email
if st.session_state.get('email_input') and '@' in st.session_state.get('email_input'):
    st.session_state['email'] = st.session_state['email_input']
    st.session_state['email_valid'] = True
    
    # Inizializza la sessione e salva l'ID
    if 'session_id' not in st.session_state:
        st.session_state['session_id'] = data_storage.initialize_session(st.session_state['email'])
...
if switch_page and st.session_state.get('email_valid'):
